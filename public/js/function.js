...
function gnbOpen(){
    ...
    tl.to(gnb, .1 , {display:'block'})
    tw.to(gnb, .4 , {height:'100vh',ease:Quint.easeInOut})
    tw.to(gnb, .8 , {'max-width':'100%', ease:Quint.easeInOut})
    tw.staggerTo($('.site-menu li a'), .8, {alpha:1, 'margin-top':0,  ease:Quint.easeInOut} , .1)
    
}
function gnbClose(){
  var gnb = $('.gnb');
    tl.staggerTo($('.site-menu li a'), .5, {alpha:0, 'margin-top':'-100%',  ease:Quint.easeInOut} , .1)
            tw.to(gnb, .5 , {'max-width':'0%', ease:Quint.easeInOut , delay:0.2} ) 
            tw.to(gnb, .4 , {height:'0',ease:Quint.easeInOut ,delay:0.5}) 
            tw.to(gnb, .1 , {display:'none' , delay:0.9});
...
function pageLoad(){
         setTimeout(function(){
        tw.to($('.page-top .bg'), .6, {x:0, ease:Quint.easeInOut} );
        tw.to($('.page-top .page-img'),.6, {x:0, alpha:1, ease:Quint.easeInOut});    
        tw.to($('.page-top .bg'), .6, {x:'100%', ease:Quint.easeInOut,delay:.65} );
         tw.to($('.page-top .info-bg'), .6, {x:'0%', ease:Quint.easeInOut,delay:.65} );
     },200)
}
...
function mainAnimation(){
        ...
        tw.to($('#first-slide'),.8,{scale:1, ease:Expo.easeInOut});
        tw.to(t, .5 , {x:0, ease:Quint.easeInOut ,delay:.5});
        tw.to($('.line'),.5, {scale:1,ease:Quint.easeInOut})
        tw.to($('.load-animation .hidden'), .1 , { alpha:1, delay:.9});
        tw.to(t, .5 , {x:'110%', ease:Quint.easeInOut, delay: .9});
        tw.from($('.page-progress'), 1 , {scaleX:0,  ease:Quint.easeInOut});    
        tw.staggerTo($('.main-txt .perspective-ani span'),.8,{rotationY:'0deg',  alpha:1, ease:Power1.easeOut ,delay:1},.2);
        tw.to($('.btn-about'), .5, {alpha:1, bottom:'0px', ease:Power1.easeInOut, delay:1.8})
}
    ...
    if(sc >= $('.site-header').height()){
        ...
        $('.project-list li, .prx').each(function(){
          var i = $(this).attr('data-distance');
            $(this).css({'transform':'translate(0,'+-(ds*i)+'%)'});                
        });                
...
function scroll_Top(){
    $('body,html').stop().animate({scrollTop:0},800,'easeInOutQuint');
}
function scrollDown(){
    $('body,html').stop().animate({scrollTop:$('.project-main').offset().top},800,'easeInOutQuint');
}
...
var Partallan = (function(window){
  ...
  var W = window.innerWidth,
      ...
      renderer = PIXI.autoDetectRenderer(W, H, {view:document.getElementById("particle"), backgroundColor : 0xf4f4f4, antialias : true, résolution: 1}),
  ...
  var Partallan = {
    ...
    writeText: function(){
      var textSample = new PIXI.Text(text, { font: 'bold 300px ns, sans-serif',  align: 'left' });
